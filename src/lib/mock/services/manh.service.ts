...
// ─── Mảnh theo PO — xuất đan (vật tư thành phẩm) và nhập đan (kho thành phẩm) đọc/ghi CHUNG
// 1 bản ghi ManhAllocation (theo từng điểm đan) nên luôn đồng bộ với nhau — không phải 2 hệ thống
// độc lập. 1 mảnh có thể xuất cho nhiều điểm đan khác nhau, mỗi điểm đan có luỹ kế xuất/nhập riêng.
// Bảng này độc lập với tồn kho mfgWarehouseItems (không liên quan tới chuyển kho nội bộ phôi sơn
// hàn → vật tư thành phẩm, vốn vẫn dùng cơ chế cũ). ──────────────────────────────────────────────
