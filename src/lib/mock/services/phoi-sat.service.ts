...
// DA_NHAN: kho đã xuất, đang cắt · CHO_DUYET: Phôi xác nhận, chờ KCS duyệt · DA_CAT: KCS đã duyệt
export type SatIssueStatus = 'CHO_NHAN' | 'DA_NHAN' | 'CHO_DUYET' | 'DA_CAT';
...
function seed(): SatIssue[] {
  ...
  return [
    ...
    // ── Phôi đã xác nhận, đang chờ KCS duyệt ──
    { ...J55, id: 's9', lineId: 113, loaiSat: 'Sắt Hộp 6 zem', quyCach: '25×50',
      bundles: [P25_A(10)], dotThoiGian: '2026-07-09 09:50', status: 'CHO_DUYET', soCayThuc: 10 },
...
// Phôi xác nhận cắt xong → chờ KCS duyệt (chưa tính là "đã cắt").
...
// KCS duyệt sản lượng → tính là "đã cắt" (DA_CAT). Dùng cho màn KCS sau này.
export async function duyetSanLuong(id: string) {
  await mockDelay();
  const it = issues.find((x) => x.id === id);
  if (it && it.status === 'CHO_DUYET') {
    it.status = 'DA_CAT';
    it.hoanThanhAt = now();
  }
  return { id, status: it?.status };
