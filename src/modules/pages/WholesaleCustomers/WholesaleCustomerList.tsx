...
import {
  getWholesaleCustomers, getWholesaleCustomerDetail,
  createWholesaleCustomer, updateWholesaleCustomer, addWholesaleCareHistory,
  assignWholesaleCustomerSales, getSalesUsers,
} from '../../../services/api'
import type { WholesaleCustomer, WholesaleCareHistoryItem, Order, SalesUserSummary } from '../../../types'
import { Plus, X, ChevronRight, MessageSquare, ClipboardList, UserCheck } from 'lucide-react'

// ─── Add/Edit Form Modal ────────────────────────────────────────────────────

type FormData = {
  businessName: string; taxCode: string; businessLicense: string;
  representativeName: string; phone: string; email: string;
  address: string; region: string; debt: string; note: string;
}

const EMPTY: FormData = {
  businessName: '', taxCode: '', businessLicense: '',
  representativeName: '', phone: '', email: '',
  address: '', region: '', debt: '', note: '',
}

function WholesaleFormModal({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (initial) await updateWholesaleCustomer(initial.id, payload)
      else await createWholesaleCustomer(payload)
      onSaved()
      onClose()
    } finally { setSaving(false) }
  }

  const isValid = form.businessName && form.representativeName && form.phone && form.address

  const fields: { label: string; key: keyof FormData; placeholder: string; required?: boolean }[] = [
    { label: 'Tên doanh nghiệp *', key: 'businessName', placeholder: 'Công ty TNHH ABC', required: true },
    { label: 'Người đại diện *', key: 'representativeName', placeholder: 'Nguyễn Văn A', required: true },
    { label: 'Số điện thoại *', key: 'phone', placeholder: '0901234567', required: true },
    { label: 'Email', key: 'email', placeholder: 'contact@company.com' },
    { label: 'Mã số thuế', key: 'taxCode', placeholder: '0123456789' },
    { label: 'Giấy phép KD', key: 'businessLicense', placeholder: 'GP số...' },
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
      <div style={{ background: 'var(--surface)', borderRadius: 10, width: 600, maxHeight: '90vh', overflowY: 'auto', padding: 28, boxShadow: '0 20px 60px rgba(0,0,0,.3)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div style={{ fontWeight: 700, fontSize: 16 }}>{initial ? 'Cập nhật khách sỉ' : 'Thêm khách hàng sỉ'}</div>
        ...
        <div style={{ marginTop: 14 }}>
          ...
          <input value={form.address} onChange={set('address')} placeholder="Số nhà, đường, quận/huyện, tỉnh/thành..." style={{ width: '100%' }} />
        </div>
...
function DetailPanel({
  customerId, onClose, onEdit, onRefreshList, isManager,
}: {
  customerId: number
  onClose: () => void
  ...
  const fetchDetail = useCallback(() => getWholesaleCustomerDetail(customerId), [customerId])
  const { data: customer, isLoading, refetch } = useFetch<WholesaleCustomer>(fetchDetail)

  const handleAddNote = async () => {
    if (!note.trim()) return
    setSubmitting(true)
    try {
      await addWholesaleCareHistory(customerId, note.trim())
      setNote('')
      refetch()
      onRefreshList()
  ...
  return (
    <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 440, background: 'var(--surface)', borderLeft: '1px solid var(--border)', boxShadow: '-4px 0 20px rgba(0,0,0,.12)', zIndex: 100, display: 'flex', flexDirection: 'column' }}>
      {/* Header */}
      <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          {isLoading
            ...
            : <>
              <div style={{ fontWeight: 700, fontSize: 16 }}>{customer?.businessName}</div>
              <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 2 }}>{customer?.representativeName} · {customer?.phone}</div>
            </>
          }
        </div>
        ...
        <>
          ...
          <div style={{ padding: '14px 20px', borderBottom: '1px solid var(--border)', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px', fontSize: 12 }}>
            {[
              ...
              ['Giấy phép KD', customer.businessLicense || '—'],
            ...
            {customer.note && (
              <div style={{ gridColumn: '1 / -1' }}>
                <div style={{ color: 'var(--text3)', fontSize: 11, marginBottom: 2 }}>Ghi chú</div>
                <div style={{ fontWeight: 400, color: 'var(--text2)' }}>{customer.note}</div>
              </div>
            )}
          </div>
            ...
            {activeTab === 'care' && (
              <>
                <div style={{ marginBottom: 16 }}>
                  <textarea value={note} onChange={e => setNote(e.target.value)} rows={2} placeholder="Ghi chú cuộc gọi, đàm phán, nội dung hợp đồng..." style={{ width: '100%', padding: '8px 10px', border: '1px solid var(--border)', borderRadius: 'var(--radius)', fontSize: 12, resize: 'vertical', marginBottom: 8 }} />
                ...
                {(customer.careHistory ?? []).length === 0
                  ...
                  : (customer.careHistory ?? []).map((h: WholesaleCareHistoryItem) => (
...
export default function WholesaleCustomerList() {
  ...
  const { data: customers, isLoading, error, refetch } = useFetch<WholesaleCustomer[]>(getWholesaleCustomers)
  const { data: salesUsers } = useFetch<SalesUserSummary[]>(
    () => isManager ? getSalesUsers() : Promise.resolve([])
  )
  const wholesaleSales = (salesUsers ?? []).filter((u: SalesUserSummary) => u.salesType === 'WHOLESALE')

  const [selectedId, setSelectedId] = useState<number | null>(null)
  const [formCustomer, setFormCustomer] = useState<WholesaleCustomer | 'new' | null>(null)
  const [assigningId, setAssigningId] = useState<number | null>(null)

  const handleAssign = async (customerId: number, salesId: number) => {
    await assignWholesaleCustomerSales(customerId, salesId)
    setAssigningId(null)
    refetch()
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: 18 }}>Khách hàng sỉ</div>
          <div style={{ fontSize: 12, color: 'var(--text3)', marginTop: 2 }}>{list.length} đối tác</div>
        </div>
        {isManager && (
          <button className="primary" onClick={() => setFormCustomer('new')} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <Plus size={14} /> Thêm khách sỉ
          </button>
        )}
      </div>
      ...
      <div className="card" style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
          <thead>
            <tr style={{ borderBottom: '1px solid var(--border)', background: 'var(--surface2)' }}>
              {(['Tên doanh nghiệp', 'Người đại diện', 'SĐT', 'Khu vực', isManager ? 'Nhân viên CSKH' : null, 'Số đơn', 'Thực thu', 'Công nợ', ''] as (string | null)[])
          ...
          <tbody>
            {list.map((c: WholesaleCustomer) => (
              <tr key={c.id} style={{ borderBottom: '1px solid var(--border)' }}
                onMouseEnter={e => (e.currentTarget.style.background = 'var(--surface2)')}
                onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
                <td style={{ padding: '10px 12px', fontWeight: 600, fontSize: 13, color: 'var(--blue)', cursor: 'pointer' }}
                  onClick={() => setSelectedId(c.id)}>{c.businessName}</td>
                <td style={{ padding: '10px 12px', fontSize: 13 }}>{c.representativeName}</td>
                <td style={{ padding: '10px 12px', fontSize: 12 }}>{c.phone}</td>
                <td style={{ padding: '10px 12px', fontSize: 12 }}>{c.region || '—'}</td>
                  ...
                  <td style={{ padding: '10px 12px' }}>
                    {assigningId === c.id ? (
                      <select autoFocus defaultValue=""
                        ...
                        {wholesaleSales.map((s: SalesUserSummary) => <option key={s.id} value={s.id}>{s.name}</option>)}
                      </select>
                    ) : c.assignedSales ? (
                ...
                <td style={{ padding: '10px 12px', fontSize: 12, textAlign: 'center' }}>{c.orderCount ?? 0}</td>
        ...
        {list.length === 0 && (
          <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text3)', fontSize: 13 }}>Chưa có khách sỉ nào</div>
        )}
      </div>
