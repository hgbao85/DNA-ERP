...
import { ChevronDown, ChevronUp, Send, Award, PackageCheck, Truck, Plus, X } from 'lucide-react'
...
function CompareModal({ item, suppliers, onClose, onChoose }: {
  ...
  return (
    ...
    >
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--surface)', borderRadius: 12, width: '100%', maxWidth: 520, padding: 24, boxShadow: '0 8px 32px rgba(0,0,0,.2)' }}>
        <h3 style={{ margin: '0 0 4px', fontSize: 16, fontWeight: 700 }}>Gợi ý NCC — {item.materialName}</h3>
        <div style={{ fontSize: 12, color: 'var(--text3)', marginBottom: 14 }}>
          Cần mua {item.buyQty} {item.unit} · Chọn NCC từ danh sách hoặc nhập tay ở bảng chính
        </div>
        {list.length === 0 ? (
